  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (rand_uniform(0, 1) < add_replay) {
      for (int k = 0; k < REPLAY_K; ++k) {
        ...
        memcpy(s_a_ns_dr_additional->data+goal_offset_1, ag->data, AG_DIM*sizeof(float));
        memcpy(s_a_ns_dr_additional->data+goal_offset_2, ag->data, AG_DIM*sizeof(float));
