...
int perform_update(model* m, float learning_rate) {
  if (learning_rate > 0) {
    switch (m->opt.type) {
      case sgd:
        m->opt.cache.s.learning_rate = learning_rate;
        break;
      case adam:
        m->opt.cache.a.learning_rate = learning_rate;
        break;
      default:
        break;
    }
...
static int sgd_update(model* m) {
  float learning_rate = m->opt.cache.s.learning_rate;
  for(int i = 0; i < m->num_of_layers; i++) {
...
static int adam_update(model* m) {
  ...
  // timer_reset(&start);
  m->opt.cache.a.timestamp++;
  adam_optimizer optimizer = m->opt.cache.a;
