...
int initialize_training_env(model* m, int batch_size) {
  ...
  */
  ...
  global_config.mem_objs.push_back(Named_buffer("params_T", context, CL_MEM_READ_WRITE, m->param_size*sizeof(float), NULL));
...
int fpga_backward(model* m, matrix_t* grad) {
  ...
  cl_mem tmp;
  enqueue_linear_backward_prop(linear_backward_prop, bp_queue, &params_T, &param_offset, &dims, &input_buffer, &input_r, &input_c, &cache_T, &cache_offset, &output_buffer, &output_r, &output_c, &param_grads, &err_code, &linear_bp_event);
