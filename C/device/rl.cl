...
void matmul(__global const float* restrict x,
  ...
  // for (int i = 0; i < l_r_col*l_r_row; ++i) {
  //   printf("%e  ",  r[i+r_offset]);
  // }
  // printf("\n");
  // printf("======W======\n");
  *r_row = l_r_row;
...
__kernel void transpose_params_n_cache(__global   float* restrict params,
  ...
  for (int n = 0; n < num_layers; ++n) {
    ...
    for (int i = 0; i < W_r; ++i) {
      for (int j = 0; j < W_c; ++j) {
        params_T[i*W_r+j+param_offset] = params[j*W_c+i+param_offset];
      }
    }
...
__kernel void linear_backward_prop(__global const float* restrict params_T,
  ...
  matmul(params_T, b_r, b_c, *layer_param_offset, input_grad, *input_grad_r, *input_grad_c, 0, param_grads, 0, 0, *layer_param_offset);
  *layer_param_offset -= b_r * b_c;
  // int W_offset = *layer_param_offset;
  matmul(cache_T, W_c, *input_grad_r, *cache_offset, input_grad, *input_grad_r, *input_grad_c, 0, param_grads, 0, 0, *layer_param_offset);
  matmul(input_grad, *input_grad_r, *input_grad_c, 0, params_T, W_r, W_c, *layer_param_offset, output_grad, output_grad_r, output_grad_c, 0);
  *layer_param_offset -= W_r * W_c;
  *cache_offset -= W_r * W_c;
}
